...
int main(int argc, char *argv[])
{
    ...
    if(aux!=0){
	  ...
	  signal(SIGALRM, problem);
	  alarm(1);

if(atoi(argv[1])>255) printf("{\"message\": \"overflow(range is [1...255])\"}");
else{
    while(nevt<atoi(argv[1]))
    {
    	gl.fkregs[0] = 0x0;
...
int read_evt_init()
{
  ...
  fd=open("/dev/mem",O_RDWR | O_SYNC);
 	  ...
 	  gl.fkregs_size=size;
 	  gl.fkregs=(uint32_t *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd,0x43c40000);    //TEST_CONTROL_BASE
 	  // fkregs=(uint32_t *)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, fd,0);    //TEST_CONTROL_BASE
 	  if(gl.fkregs==MAP_FAILED){
 	    printf("Error - while trying to map the Registers\n");
 	    exit(1);
 	  }
 	  close(fd); /*it is not needed to keep opened */
